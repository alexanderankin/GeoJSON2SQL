...
async function polygons(knex, field, id) {
  var polygons = await knex(tbl).where({ [field]: id });
  return await Promise.all(polygons.map(pg => {
    return multipoint.multipoints(knex, 'Polygon_id', pg.id);
  }));
}

async function oneAsGeojson(knex, id) {
  var coordinates = await multipoint.multipoints(knex, 'Polygon_id', id);
